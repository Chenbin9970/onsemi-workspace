...
struct gapm_set_dev_config_cmd devConfigCmd =
{
    ...
    .addr.addr = APP_BD_ADDRESS_PERIPHERAL,
    .irk.key = APP_IRK,
    .addr_type = APP_BD_ADDRESS_PERIPHERAL_TYPE,
...
struct gapm_start_connection_cmd connectionCmd =
{
    .op = {
    ...
    .peers[0] = {
        .addr.addr = APP_BD_ADDRESS_CENTRAL1,
        .addr_type = APP_BD_ADDRESS_CENTRAL_TYPE1
     },
    .peers[1] = {
        .addr.addr = APP_BD_ADDRESS_CENTRAL2,
        .addr_type = APP_BD_ADDRESS_CENTRAL_TYPE2
     },
    .peers[2] = {
        .addr.addr = APP_BD_ADDRESS_CENTRAL3,
        .addr_type = APP_BD_ADDRESS_CENTRAL_TYPE3
     },
    .peers[3] = {
         .addr.addr = APP_BD_ADDRESS_CENTRAL4,
         .addr_type = APP_BD_ADDRESS_CENTRAL_TYPE4
     },
};
...
extern uint8_t connection_Mode;
...
void APP_SetConnectionCfmParams(uint8_t conidx, struct gapc_connection_cfm* cfm)
{
	...
	PRINTF("  connectionCfm->pairing_lvl = %d \n\r", cfm->pairing_lvl);
}
...
void Device_Param_Prepare(app_device_param_t *param)
{
    ...
    if(param->device_param_src_type == APP_PROVIDED)
    {
        ...
        uint8_t temp_periph_addr[6] = APP_BD_ADDRESS_PERIPHERAL;
        uint8_t temp_cent_addr[6] = APP_BD_ADDRESS_CENTRAL;

        if(connection_Mode == APP_BLE_PERIPHERAL_ROLE)
        {
            memcpy(param->bleAddress, temp_periph_addr , 6);
        }
        else
        {
            memcpy(param->bleAddress, temp_cent_addr , 6);
        }
