...
const struct att_db_desc att_db[] =
{
    ...
    CS_CHAR_USER_DESC(CS_TX_LONG_VALUE_USR_DSCP0,
            sizeof(CS_RX_CHAR_LONG_NAME) - 1,
            CS_RX_CHAR_LONG_NAME,
            NULL),

    /* From the BLE long transfer */
    CS_CHAR_UUID_128(CS_RX_LONG_VALUE_CHAR0,
            CS_RX_LONG_VALUE_VAL0, CS_CHAR_LONG_TX_UUID,
            PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE)| PERM(WRITE_COMMAND, ENABLE),
            sizeof(app_env_cs.from_air_buffer_long),
            app_env_cs.from_air_buffer_long,
            CUSTOMSS_RXLongCharCallback),
    CS_CHAR_CCC(CS_RX_LONG_VALUE_CCC0,
            app_env_cs.from_air_cccd_value_long,
            NULL),
    CS_CHAR_USER_DESC(CS_RX_LONG_VALUE_USR_DSCP0,
                         ...
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    switch (msg_id)
    {
        ...
        case CUSTOMSS_NTF_TIMEOUT:
        {
            ...
            if ((app_env_cs.to_air_cccd_value[0] == ATT_CCC_START_NTF && app_env_cs.to_air_cccd_value[1] == 0x00)
