...
extern struct gapm_start_connection_cmd connectionCmd;
extern struct gapm_start_advertise_cmd advertiseCmd;

/* Variable used to determine if Central or Peripheral Role */
                            ...
                            ke_task_id_t const src_id)
{
    switch(msg_id)
    {
        case GAPM_CMP_EVT:
        {
            ...
            else if((p->operation == GAPM_CONNECTION_AUTO || p->operation == GAPM_ADV_UNDIRECT)
                    && p->status == GAP_ERR_CANCELED)
            {
                /* If connection/advertise operation is cancelled, begin inverse Role */
                APP_StartAirOperation(connection_Mode);
            }
        }
        break;
        ...
        case GAPM_PROFILE_ADDED_IND:
        {
            ...
            {
                /* Start Advertising and set initial mode to Peripheral */
                ...
                /* Start the periodic timer keeping track of when to switch roles */
                      ...
                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    switch(msg_id)
    {
        case GAPC_CONNECTION_REQ_IND:
        {
            ...
            /* Ensure the device is currently advertising/connecting to trigger the
             * proper CancelCmd handler */
            APP_StartAirOperation(connection_Mode);
        }
        break;
 ...
 *                                       ke_task_id_t const src_id)
 * ----------------------------------------------------------------------------
 * Description   : Handles a timeout received when the duration between
 * 				   switching modes is reached. Calls cancel command on current
 * 				   mode.
 * Inputs        : - msg_id     - kernel message ID number
 *                 - param      - message parameter (unused)
 *                 - dest_id    - destination task ID number
...
void APP_SwitchRole_Timeout(ke_msg_id_t const msg_id, void const *param,
                           ...
                           const src_id)
{
    /* Call the GAPM cancel command. Indicates a role switch timeout and acts
     * to trigger the CancelCmd Handler */
    connection_Mode = 1 - connection_Mode;
    if (GAP_GetEnv()->gapmState == GAPM_STATE_STARTING_CONNECTION ||
        GAP_GetEnv()->gapmState == GAPM_STATE_ADVERTISING)
    {
        GAPM_CancelCmd();
    }
...
/* ----------------------------------------------------------------------------
 * Function      : void APP_StartAirOperation(uint8_t currentRole)
 * ----------------------------------------------------------------------------
 ...
 * Inputs        : None
 * Outputs       : None
 * Assumptions   : connection_Mode global has been used to monitor state
 * ------------------------------------------------------------------------- */
void APP_StartAirOperation(uint8_t currentRole)
{
    /* Finds the role the device is being requested to function in. Starts
     * operations for mode being set */
    if(GAPC_GetConnectionCount() < APP_NB_PEERS)
    {
        if(currentRole == APP_BLE_PERIPHERAL_ROLE)
        {
            PRINTF("\n\r__START ADVERTISING APP_BLE_PERIPHERAL_ROLE\n");
            GAPM_StartAdvertiseCmd(&advertiseCmd);
            connection_Mode = APP_BLE_PERIPHERAL_ROLE;
        }
        else
        {
            PRINTF("\n\r__START CONNECTION APP_BLE_CENTRAL_ROLE\n");
            GAPM_StartConnectionCmd(&connectionCmd);
            connection_Mode = APP_BLE_CENTRAL_ROLE;
        }
    }
}
