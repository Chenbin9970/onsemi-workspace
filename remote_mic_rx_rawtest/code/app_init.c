...
/* Test: 1 kHz sine at 16 kHz (16 samples/period). int32_t slots, low 16 bits
   are the sample, matching Buffer.output's layout for the ch3 DMA source. */
int32_t pcm_asrc_sine_16k[FRAME_LENGTH];

#if (OUTPUT_INTRF == PCM_TX_RAW_OUTPUT)
/* PCM output buffer. Each 32-bit word = two 16-bit samples. LIN DMA (ch5)
   re-arms via complete interrupt; no double buffering for now. */
...
#if (OUTPUT_INTRF == PCM_TX_RAW_OUTPUT)
    /* PCM: ch4 drains the 24 kHz ASRC output into pcm_tx_buf directly
       (ASRC->OUT is 32-bit = 2 samples/word, one 32-bit word per PCM frame;
       120 words per 10 ms). Not enabled here; armed at link connect so the
       boot tone on ch5 is not corrupted by a spurious ch4 run. */
...
void Initialize_Receiver_Audio_Output(void)
{
    ...
    /* DIO6 = DSP0 IRQ-rate, DIO7 = ch3 ASRC-in complete. */
    Sys_DIO_Config(6, DIO_MODE_GPIO_OUT_0);
    Sys_DIO_Config(7, DIO_MODE_GPIO_OUT_0);

    /* Fill the 16k 1 kHz sine table (16 samples/period). */
    {
...
#if 0
    /* Boot test (disabled): 24 kHz sample-rate 1 kHz sine used earlier to
       verify the PCM interface in isolation. Now the real ASRC path feeds
       pcm_tx_buf via ch4 (ASRC_OUT) at link connect, so this pre-fill and
       ch5/PCM startup are handled in App_Process_Connected instead. */
    {
        static const int16_t sine_1k_24k[24] = {
        ...
        {
            uint16_t a = (uint16_t)sine_1k_24k[(2 * i) % 24];
            uint16_t b = (uint16_t)sine_1k_24k[(2 * i + 1) % 24];
            pcm_tx_buf[i] = ((uint32_t)0 << 16) | (uint32_t)b;
        }
        PCM->TX_DATA = pcm_tx_buf[0];   /* preload first word */

        Sys_DMA_ChannelConfig(PCM_DMA_NUM, RX_DMA_PCM_STEREO, 120, 0,
                              (uint32_t)&pcm_tx_buf[0], (uint32_t)&PCM->TX_DATA);
