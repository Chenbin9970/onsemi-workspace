...
/* pcm_tx_buf -> PCM->TX_DATA. One PCM_FRAME_WORDS transfer per arm; the
   complete interrupt re-arms the single buffer. Per docs: LIN (not CIRC,
   which underruns at the wrap boundary). Each 32-bit word is two 16-bit
   samples. */
...
/* ASRC->OUT -> memory (P_TO_M). ASRC->OUT is 32-bit holding TWO 16-bit
   samples, so read SRC_WORD_SIZE_32 (2 samples per beat) straight into
   pcm_tx_buf. ch5 streams each word to the PCM, which uses TX_DATA low 16
   (TX_ALIGN_LSB) -> one sample per word. One-shot linear; re-armed on
   complete. */
#define RX_DMA_ASRC_OUT                 (DMA_SRC_ASRC |             \
                                         DMA_TRANSFER_P_TO_M |      \
                                         DMA_LITTLE_ENDIAN |        \
...
/* Audio configurarion for OD (DECIMATE_BY_256 -> 3.2MHz/256 = 12.5kHz sink) */
...
extern int32_t pcm_asrc_sine_16k[FRAME_LENGTH];

#if (OUTPUT_INTRF == PCM_TX_RAW_OUTPUT)
extern uint32_t pcm_tx_buf[PCM_FRAME_WORDS];
