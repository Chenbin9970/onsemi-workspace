...
void APP_SetAdvScanData(void)
{
    ...
    uint8_t buff_hisyncID[] = APP_ASHA_HI_SYNC_ID;
    uint8_t companyID[] = APP_COMPANY_ID;

    struct asha_adv_data_t app_asha_adv =
    {
        .service_uuid = ASHA_SERVICE_UUID,
        .protocol_version = APP_ASHA_VERSION,
        .capability = APP_ASHA_DEVICE_CAPABILITIES,
    };

    memcpy(&app_asha_adv.hisyncID, buff_hisyncID,
            sizeof(app_asha_adv.hisyncID));
    ...
    GAPM_AddAdvData(GAP_AD_TYPE_SERVICE_16_BIT_DATA, (uint8_t*) &app_asha_adv,
                    sizeof(app_asha_adv), advertiseCmd.info.host.adv_data,
                    &advertiseCmd.info.host.adv_data_len);
