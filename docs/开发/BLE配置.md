...
| 广播间隔 | **100ms** (160 × 0.625ms) | `CFG_ADV_INTERVAL_MS = 100` |
| 设备名称 | `"cbtest"` | 广播数据中可见 |
...
| Slave Latency | **10** | 最多跳过 10 个连接事件 |
| 监督超时 | **32s** (3200 × 10ms) | BLE 规范上限 |
| 有效通信间隔 | **5.5s** | 500ms × (1 + 10) = 5.5s |
...
| 参数 | 原始 Demo | 当前 | 功耗影响 |
|------|----------|------|----------|
| 广播间隔 | 40ms | **100ms** | 适配按键轮询 |
| 连接间隔 | 10ms | **500ms** | CPU 有更多睡眠窗口 |
| Latency | 0 | **10** | 跳过 10 个事件才超时 |
| 超时 | 2s | **32s** | 容忍极长时间无通信 |
