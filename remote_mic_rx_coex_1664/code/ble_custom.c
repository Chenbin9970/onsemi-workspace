        ...
        if (status == GAP_ERR_NO_ERROR)
        {
            switch (attnum)
            {
                case REMPRO_IDX_ROLE_VALUE_VAL:
                    ...
                    /* 0xFE = 清缓存并重启，重新从 7100 读取参数（参照 sleep CS 0xFE）。
                     * 重启后 cache_try_load 命不中 → 走 I2C 读回 → 重新落盘。 */
                    if (param->length >= 1 && param->value[0] == 0xFE)
                    {
                        PRINTF("[7100] 0xFE: invalidate cache + reset to reload\r\n");
                        dsp_7100_cache_invalidate();
                        NVIC_SystemReset();
                        break;   /* 不送入 rempro 重装缓冲 */
                    }
