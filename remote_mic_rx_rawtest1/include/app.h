...
#define ASRC_DITHER                     1   /* 1=加 dither 打破 ASRC 极限环 */
...
/* ASRC->OUT -> pcm_tx_buf (P_TO_M). ASRC->OUT is a 16-bit mono sample; DEST16
   writes the low 16 bits of each 32-bit word (high 16 zero-filled) -> one
   sample per word. One-shot linear; re-armed on complete. */
#define RX_DMA_ASRC_OUT                 (DMA_SRC_ASRC |             \
                                         DMA_TRANSFER_P_TO_M |      \
                                         DMA_LITTLE_ENDIAN |        \
                                         DMA_COMPLETE_INT_ENABLE |  \
